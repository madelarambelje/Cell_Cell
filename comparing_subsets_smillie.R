...
CellChatDB.use <- CellChatDB # or simply use the default CellChatDB
...
#> 2D visualization of signaling networks from datasets 1 2
# Identify signaling groups based on structure similarity using same steps:
...
ht3 = netAnalysis_signalingRole_heatmap(object.list[[i]], pattern = "incoming", 
                                        signaling = pathway.union, 
                                        title = names(object.list)[i], width = 5, 
                                        height = 6, color.heatmap = "GnBu")
ht4 = netAnalysis_signalingRole_heatmap(object.list[[i+1]], pattern = "incoming", 
                                        signaling = pathway.union, 
                                        title = names(object.list)[i+1], width = 5, 
                                        height = 6, color.heatmap = "GnBu")
...
cellchat@meta$datasets = factor(cellchat@meta$datasets, 
                                levels = c("healthy", "inflamed")) # set factor level
