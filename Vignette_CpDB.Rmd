...
```
## Installation
First, lets clone the repository, if you have already done that, skip this codechunk.
```{bash , eval=F}
...
CellphoneDB is now ready to be used, however we have to do some preprocessing to run the actual analysis 
```{r, eval=FALSE}
...
Now we are ready to run the analysis using the 'RunCPDB.sh' script. * This script requires three inputs:    
  1.    Output directory name   
  2.    Seurat object saved as rds     
  3.    Amount of threads to use    

Move to the directory this script is stored in.
```{bash, eval=FALSE}
cd Cell_Cell/
git checkout andre
...
Now we create our own heatmap! Load the count_network data.
```{r, eval=FALSE}
network = read.csv("/students/2020-2021/master/CellPhoneDB/TestingPipeline/ResultsPipeline/CellChat/out/out/count_network.txt", sep = "\t", header = TRUE)
```
### Heatmap log(counts)
